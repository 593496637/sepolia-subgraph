...
export function handleTransferRecord(event: TransferRecordEvent): void {
  // 记录事件处理开始，便于调试和监控
  log.info("Processing TransferRecord event from {} to {} with transferValue {}", [
    event.params.from.toHexString(), // 发送方地址
    event.params.to.toHexString(), // 接收方地址
    event.params.transferValue.toString(), // 转账金额
  ]);
  ...
  transferRecord.value = event.params.transferValue; // 转账金额
